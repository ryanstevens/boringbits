...
module.exports = async function initRoutes(BoringInjections) {
  ...
  const moduleData = await (0, _requireInjectAll.default)([_paths.default.boring_routers, _paths.default.server_routers], boring);
  ...
  methods[method].handler = function wrappedHandler(req, res, next) {
    ...
    boring.perform(`http::${method.toLowerCase()}::middleware`, ctx, () => new Promise(function (resolve, reject) {
      ...
      boring.perform(`http::${method.toLowerCase()}`, ctx, async () => {
        ...
        _boringLogger.default.debug(e, `There was rejection from a {{http::${method}}} interceptor for ` + url);
      });
    }).catch(e => {
      _boringLogger.default.debug(e, `There was rejection from a {{http::${method}::middleware}} interceptor for ` + url);
    });
  };
}
