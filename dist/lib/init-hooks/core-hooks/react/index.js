...
module.exports = function reactHook(BoringInjections) {
  ...
  decorators.router.reactEntry = function shadowedEntrypoint(options = {}) {
    ...
    return function createDecorator(target, field, descriptor) {
      ...
      const reactHandlerPaths = _objectSpread({
      ...
      const entryPointPath = options.app_dir + '/' + reactHandlerPaths.entrypoint; // check to make sure entrypoint is real

      if (!fs.existsSync(entryPointPath + '.js') && !fs.existsSync(entryPointPath + '.jsx') && !fs.existsSync(entryPointPath + '.ts') && !fs.existsSync(entryPointPath + '.tsx')) {
      ...
      const entrypointPaths = [beforeEntry, reactHandlerPaths.entrypoint, afterEntry].filter(Boolean);
      return decorators.router.entrypoint(...entrypointPaths)(target, field, shadowedReactEntry(reactHandlerPaths)(target, field, descriptor));
    };
  };
