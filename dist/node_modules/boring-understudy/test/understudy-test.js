...
describe('Understudy Promisified', function () {
  it('will return a promise from perform', done => {
    ...
    tester.before('beep', function (localOptions) {
      ...
      localOptions.transport = 'walk'; // does not need to return a promise, it will just be wrapped and returned sync
    });
  ...
  it('will call the catch if a before rejects', done => {
    ...
    }).then(() => assert.fail('Should not be resolved')).catch(e => {
  ...
  it('will call the catch if thrown on after', done => {
    ...
    tester.perform('beep', function (next) {// no op
    }).then(() => assert.fail('Should not be resolved')).catch(e => {
      caught = true;
...
//# sourceMappingURL=understudy-test.js.map