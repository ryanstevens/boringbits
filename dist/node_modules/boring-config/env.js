...
if (!process.env.NODE_ENV) {
  ...
  process.env.NODE_ENV = 'development'; // throw new Error(
  //   'The NODE_ENV environment variable is required but was not specified.'
  // );
}

const NODE_ENV = process.env.NODE_ENV;
const dotenv = process.cwd() + '/.env'; // https://github.com/bkeepers/dotenv#what-other-env-files-can-i-use

var dotenvFiles = [`${dotenv}.${NODE_ENV}.local`, `${dotenv}.${NODE_ENV}`, // Don't include `.env.local` for `test` environment
// since normally you expect tests to produce the same
// results for everyone
NODE_ENV !== 'test' && `${dotenv}.local`, dotenv].filter(Boolean); // Load environment variables from .env* files. Suppress warnings using silent
